...
export const getCafeteriaForm = (currentPage = 1) => async (dispatch) => {
    ...
    try {
        ...
        //then get all data 
        const { data } = await newRequest.get(`/api/v1/admin/numbers?page=${currentPage}`)
...
export const newCafeteriaForm = (agentData) => async (dispatch) => {
    try {
        ...
        const { data } = await newRequest.post(`/api/v1/admin/numbers/new`, agentData, config)
...
export const deleteCafeteriaForm = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await newRequest.delete(`/api/v1/admin/numbers/${id}`)
