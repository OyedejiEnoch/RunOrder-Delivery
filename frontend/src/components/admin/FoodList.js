...
import { getAdminFoods, clearErrors, deleteProduct } from "../../action/foodActions";
import Sidebar from "./Sidebar";
import { DELETE_PRODUCTS_RESET } from "../../constants/foodConstants";
...
function FoodList() {
    ...
    const { loading, error, products } = useSelector(state => state.foodProducts);
    const { error: deleteError, isDeleted } = useSelector(state => state.foodProduct)
        ...
        products.forEach(product => {
            data.rows.push({
               name: product.name,
                price: `${product.price}`,
                actions: <Fragment>
                    <Link to={`/admin/foods/${product._id}`} className="btn btn-primary py-1 px-2">
