...
import {clearErrors, deleteCafeteriaForm} from "../../action/nationalKitchionActions"
import { DELETE_FORM_RESET } from "../../constants/nationalKitchenConstants";
import 'animate.css';

const NkDisplayForm = ({forms}) => {

    
  const dispatch = useDispatch()
  const navigate = useNavigate()

  const { error: deleteError, isDeleted } = useSelector(state => state.nationalKitchen)
  ...
  useEffect(() => {
      ...
      if (isDeleted) {
          ...
          navigate("/admin/nationalKitchen")
